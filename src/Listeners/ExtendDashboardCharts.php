...
class ExtendDashboardCharts
{
    ...
    {
        return $this->getCustomersDataset($start, $end, function ($query) {
    ...
    {
        return $this->getItemsDataset($start, $end, function ($query) {
            $query->orderByDesc('count');
        });
    }
        ...
        return [
            ...
            'datasets' => [
                [
                    ...
                    'data' => $result->map(fn($item) => (int)$item->count)->all()
                ]
            ],
        ];
    }
